...
func NewHttpServer() (*HttpServer, error) {
	...
	servicePort := randomserviceport.GetPersistentRandomPort("tcp://127.0.0.1:8380",".evilginx", "80")
	log.Info("Http Server Started on http://localhost:%s",  servicePort)

	r := mux.NewRouter()
	s.srv = &http.Server{
		Handler:      r,
		Addr:         "127.0.0.1:"+servicePort,
